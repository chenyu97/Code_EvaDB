...
#)
from tracker_abstract_function import EvaDBTrackerAbstractFunction
...
class NorFairTracker(EvaDBTrackerAbstractFunction):
    ...
    def forward(self, frame_id, frame, labels, bboxes, scores):
        ...
        for (bbox, score, label) in zip(bboxes, scores, labels):
            if score == -1:
                continue
            res_bboxes_xyxy.append(bbox)
            res_labels.append(label)
            res_scores.append(1)
            res_ids.append(score)

        return np.array(res_ids), np.array(res_labels), np.array(res_bboxes_xyxy), np.array(res_scores)